...
export class BankMySuffixDeleteDialogComponent {
    ...
    constructor(private bankService: BankMySuffixService, public activeModal: NgbActiveModal, private eventManager: JhiEventManager) {}
...
export class BankMySuffixDeletePopupComponent implements OnInit, OnDestroy {
    ...
    ngOnInit() {
        this.activatedRoute.data.subscribe(({ bank }) => {
            setTimeout(() => {
                this.ngbModalRef = this.modalService.open(BankMySuffixDeleteDialogComponent as Component, {
                    size: 'lg',
                    backdrop: 'static'
                });
